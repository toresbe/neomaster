...
namespace GUI {
	...
	void ModuleWheel::handle_panel_input(const Panel::input_damage_t & damage, const Panel::input_state_t & state) {
		if (damage.wheel_3)
		{
			this->input_state = state;
			Modules::NeomasterModuleUI * new_module = new_selected_module(state.posWheel3);
	...
	void ModuleWheel::add_module_bitmap_entry(Modules::NeomasterModuleUI *module, const SDL_Rect &rect) {
	...
	ModuleWheel::ModuleWheel(SDL_Renderer * renderer, Modules::ui_module_list_t module_list) {
